...
namespace Imagine::Vulkan {
	...
	VulkanRenderer::VulkanRenderer(const RendererParameters& renderParams, const ApplicationParameters& appParams) : Renderer(), m_RenderParams(renderParams),m_AppParams(appParams)
	{
		InitialiseVulkan();
		InitialiseSwapchain();
		InitialiseCommands();
		InitialiseSyncStructures();
	}
...
#if defined(MGN_WINDOW_SDL3)
		SDL_Vulkan_CreateSurface(reinterpret_cast<struct SDL_Window*>(Core::Window::Get()->GetWindowPtr()), m_Instance, nullptr, &m_Surface);
#elif
	...
	void VulkanRenderer::CreateSwapChain(const uint32_t width, const uint32_t height)
	{
		...
		vkb::Swapchain vkbSwapchain = swapchainBuilder
			...
			.set_desired_present_mode(VK_PRESENT_MODE_FIFO_KHR) // Hard VSync. Will limit the refreshrate to the one of the monitor.
