...
namespace Imagine {
	...
	public:
		...
		public:
			iterator() = default;
			~iterator() = default;
			iterator(const int64_t index, LoopBackBuffer &buffer) :
				buffer(&buffer), index(index % Capacity) {}
		...
		public:
			...
			[[nodiscard]] iterator operator+(const int64_t offset) const {
				auto tmp = *this;
				tmp += offset;
				return tmp;
			}
			...
			[[nodiscard]] iterator operator++(int) const {
				auto tmp = *this;
				++(*this);
				return tmp;
			}
		...
		public:
			...
			~const_iterator() = default;
			const_iterator(const int64_t index, const LoopBackBuffer &buffer) :
				buffer(&buffer), index(index % Capacity) {}
			explicit const_iterator(const iterator &iterator) :
				buffer(iterator.get_buffer()), index(buffer->get_index() % Capacity) {}
		...
		public:
			...
			[[nodiscard]] const_iterator operator+(const int64_t offset) const {
				auto tmp = *this;
				tmp += offset;
				return tmp;
			}
			...
			[[nodiscard]] const_iterator operator++(int) const {
				auto tmp = *this;
				++(*this);
				return tmp;
			}
