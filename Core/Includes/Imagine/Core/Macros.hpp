...
#if MGN_USE_ASSERT
	#define MGN_CORE_ASSERT(condition, ...) if(!(condition)) { MGN_CORE_ERROR(__VA_ARGS__); MGN_BREAK(); }
	#define MGN_ASSERT(condition, ...) if(!(condition)) { MGN_ERROR(__VA_ARGS__); MGN_BREAK(); }
#else
...
#if MGN_DEBUG
#define MGN_CORE_CHECK(condition, ...)  if(!(condition)) { MGN_CORE_WARNING(__VA_ARGS__); }
#define MGN_CHECK(condition, ...)  if(!(condition)) { MGN_WARNING(__VA_ARGS__); }
#define MGN_CORE_CHECK_ERROR(condition, ...)  if(!(condition)) { MGN_CORE_ERROR(__VA_ARGS__); }
#define MGN_CHECK_ERROR(condition, ...)  if(!(condition)) { MGN_ERROR(__VA_ARGS__); }
#else
