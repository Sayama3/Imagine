...
namespace Imagine::Core {
	...
	BufferView::BufferView(Buffer &buffer, const uint64_t offset, const uint64_t size) :
		m_Buffer(buffer.Get() ? reinterpret_cast<uint8_t *>(buffer.Get()) + offset : nullptr), m_Size(size) {
	}

	BufferView::BufferView(void *buffer, const uint64_t offset, const uint64_t size) :
...
namespace Imagine::Core {
	...
	ConstBufferView::ConstBufferView(const void *buffer, const uint64_t offset, const uint64_t size) :
	...
	ConstBufferView::ConstBufferView(const ConstBufferView &other) :
		m_Buffer(other.m_Buffer), m_Size(other.m_Size) {
	}
