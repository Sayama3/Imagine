...
namespace Imagine::Core
{
	...
	{
		MGN_PROFILE_FUNCTION();
		//		MGN_PROFILE_SCOPE("Model::Model -> Create Position RendererBuffer");
		size_t sizeofValue = sizeof(T);
		int value = primitive.attributes[attribute];
		auto &accessor = model.accessors[value];
		auto &bufferView = model.bufferViews[accessor.bufferView];
		auto &buffer = model.buffers[bufferView.buffer];
		MGN_CORE_ASSERT(accessor.componentType == ct, "The component type should be {0} and it is {1}.", GLTF::Helper::ComponentTypeToString((GLTF::ComponentType) ct), GLTF::Helper::ComponentTypeToString((GLTF::ComponentType) accessor.componentType));
		MGN_CORE_ASSERT(accessor.type == at, "The component type should be {0} and it is {1}.", GLTF::Helper::AccessorTypeToString((GLTF::AccessorType) at), GLTF::Helper::AccessorTypeToString((GLTF::AccessorType) accessor.type));
		MGN_CORE_ASSERT(bufferView.byteLength >= accessor.count * sizeofValue, "byteLength '{0}' is not long enought.", bufferView.byteLength);
	...
	{
		MGN_PROFILE_FUNCTION();
		//		MGN_PROFILE_SCOPE("Model::Model -> GetRawBuffer");
		auto &accessor = model.accessors[value];
		auto &bufferView = model.bufferViews[accessor.bufferView];
		auto &buffer = model.buffers[bufferView.buffer];
	...
	glm::vec4 Convertu8Vec3ToVec4(const glm::u8vec3 &v) { MGN_PROFILE_FUNCTION(); return glm::vec4(glm::vec3(v) / 255.0f, 1.0f); }
	glm::vec4 Convertu16Vec3ToVec4(const glm::u16vec3 &v) { MGN_PROFILE_FUNCTION(); return glm::vec4(glm::vec3(v) / 65535.0f, 1.0f); }
	glm::vec4 Convertu32Vec3ToVec4(const glm::u32vec3 &v) { MGN_PROFILE_FUNCTION(); return glm::vec4(glm::vec3(v) / 4294967295.0f, 1.0f); }
	glm::vec4 ConvertVec3ToVec4(const glm::vec3 &v) { MGN_PROFILE_FUNCTION(); return glm::vec4(v, 1.0f); }
	...
	glm::vec4 GetColor(const std::string &attribute, tinygltf::Model &model, tinygltf::Primitive &primitive, size_t verticesCount, size_t index)
	{
		...
		MGN_CORE_ASSERT(index >= 0 && index < verticesCount, "The index {0} is not valid.", index);
		...
		if (colorAccessor.type == GLTF::AccessorType::VEC4) {
			switch ((GLTF::ComponentType) colorAccessor.componentType) {
				case GLTF::ComponentType::UnsignedByte:
					MGN_CORE_ASSERT(verticesCount == colorSize / sizeof(glm::u8vec4), "The Color RendererBuffer only has {0} items instead of {1}", colorSize / sizeof(glm::u8vec4), verticesCount);
					color = (static_cast<const glm::u8vec4 *>(colorBuffer)[index]);
					color /= glm::vec4(255.0);
					break;
				case GLTF::ComponentType::UnsignedShort:
					MGN_CORE_ASSERT(verticesCount == colorSize / sizeof(glm::u16vec4), "The Color RendererBuffer only has {0} items instead of {1}", colorSize / sizeof(glm::u16vec4), verticesCount);
					color = (static_cast<const glm::u16vec4 *>(colorBuffer)[index]);
					color /= glm::vec4(65535.0);
					break;
				case GLTF::ComponentType::UnsignedInt:
					MGN_CORE_ASSERT(verticesCount == colorSize / sizeof(glm::u32vec4), "The Color RendererBuffer only has {0} items instead of {1}", colorSize / sizeof(glm::u32vec4), verticesCount);
					color = (static_cast<const glm::u32vec4 *>(colorBuffer)[index]);
					color /= glm::vec4(4294967295.0);
					break;
				case GLTF::ComponentType::Float:
					MGN_CORE_ASSERT(verticesCount == colorSize / sizeof(glm::vec4), "The Color RendererBuffer only has {0} items instead of {1}", colorSize / sizeof(glm::vec4), verticesCount);
					color = (static_cast<const glm::vec4 *>(colorBuffer)[index]);
					break;
				default:
					MGN_CORE_ASSERT(false, "The type {0} is not supported.", colorAccessor.componentType);
			}
		}
		else if (colorAccessor.type == GLTF::AccessorType::VEC3) {
			switch ((GLTF::ComponentType) colorAccessor.componentType) {
				case GLTF::ComponentType::UnsignedByte:
					MGN_CORE_ASSERT(verticesCount == colorSize / sizeof(glm::u8vec3), "The Color RendererBuffer only has {0} items instead of {1}", colorSize / sizeof(glm::u8vec3), verticesCount);
					color = Convertu8Vec3ToVec4(static_cast<const glm::u8vec3 *>(colorBuffer)[index]);
				case GLTF::ComponentType::UnsignedShort:
					MGN_CORE_ASSERT(verticesCount == colorSize / sizeof(glm::u16vec3), "The Color RendererBuffer only has {0} items instead of {1}", colorSize / sizeof(glm::u16vec3), verticesCount);
					color = Convertu16Vec3ToVec4(static_cast<const glm::u16vec3 *>(colorBuffer)[index]);
				case GLTF::ComponentType::UnsignedInt:
					MGN_CORE_ASSERT(verticesCount == colorSize / sizeof(glm::u32vec3), "The Color RendererBuffer only has {0} items instead of {1}", colorSize / sizeof(glm::u32vec3), verticesCount);
					color = Convertu32Vec3ToVec4(static_cast<const glm::u32vec3 *>(colorBuffer)[index]);
				case GLTF::ComponentType::Float:
					MGN_CORE_ASSERT(verticesCount == colorSize / sizeof(glm::vec3), "The Color RendererBuffer only has {0} items instead of {1}", colorSize / sizeof(glm::vec3), verticesCount);
					color = ConvertVec3ToVec4(static_cast<const glm::vec3 *>(colorBuffer)[index]);
				default: MGN_CORE_ASSERT(false, "The type {0} is not supported.", colorAccessor.componentType);
			}
		}
	...
	{
		MGN_CORE_ASSERT(false, "Not implemented yet.");
	...
	{
		MGN_CORE_ASSERT(IsModel(metadata.FilePath), "The asset '{0}' is not a 3d model", metadata.FilePath.string());

		auto path = metadata.FilePath.GetFullPath();
		MGN_CORE_ASSERT(std::filesystem::exists(path), "The file {0} doesn't exist", path.string());
		...
		bool loaded = false;
		if (path.extension() == MGN_EXTENSION_GLTF) loaded = loader.LoadASCIIFromFile(&model, &err, &warn, path.string().c_str());
		else loaded = loader.LoadBinaryFromFile(&model, &err, &warn, path.string().c_str());// for binary glTF(.glb)

		if (!warn.empty()) {
			MGN_CORE_WARNING(warn);
		}
		...
		if (!loaded) {
			MGN_CORE_ERROR("Failed to parse glTF {0}", path.string());
			return nullptr;
		}

		auto assetManager = Project::GetActive()->GetEditorAssetManager();
		Ref<::Imagine::Core::Model> mgnModel = CreateRef<::Imagine::Core::Model>(std::vector<MeshGroup>{}, std::vector<Node>{}, std::vector<ModelRootScene>{});

		{
			MGN_PROFILE_SCOPE("Model::Model -> Create Nodes");
			mgnModel->m_Nodes.reserve(model.nodes.size());
			for (auto &node: model.nodes) {
				glm::mat4 matrix = GLTF::Helper::GetMatrix(node);
				std::vector<int> children = node.children;
				if (node.mesh > -1) mgnModel->m_Nodes.emplace_back(node.mesh, children, matrix);
				else
					mgnModel->m_Nodes.emplace_back(children, matrix);
			}
		}
		std::vector<Texture2DField> textures;
		{
			MGN_PROFILE_SCOPE("Model::Model -> Create Textures");
			textures.reserve(model.textures.size());
			for (auto &texture: model.textures) {
				MGN_PROFILE_SCOPE("Model::Model -> Create Texture");
				//TODO: remove the assert and leave a blank or magenta texture by default.
				MGN_CORE_ASSERT(texture.source > -1, "No texture associated.");
				auto &image = model.images[texture.source];
				if (image.bufferView > -1) {
					MGN_CORE_ASSERT(image.width > -1 && image.height > -1 && image.component > -1, "Cannot handle image if we don't have the size or the bits of each pixels.");
					MGN_CORE_ASSERT(image.image.size() != 0, "Cannot handle the current image...");
					int width = image.width;
					int height = image.height;
					int channels = image.component;
					if (image.bits == 8) {
						MGN_CORE_ASSERT(image.image.size() >= width * height * channels * (image.bits / 8), "The buffer is not long enought...");
						...
						MGN_CORE_ASSERT(false, "The pixel type {0} is not handled.", image.pixel_type);
					}
				}
				else {
					MGN_CORE_ASSERT(!image.uri.empty(), "The image don't have any way to be fetch.");
					MGN_CORE_ASSERT(!image.uri.starts_with("http"), "The engine cannot fetch the image from the internet for now.");
					MGN_CORE_ASSERT(image.mimeType == "image/jpeg" || image.mimeType == "image/png" || image.mimeType == "image/bmp" || image.mimeType == "image/gif" || image.uri.ends_with(".png") || image.uri.ends_with(".jpg") || image.uri.ends_with(".jepg") || image.uri.ends_with(".bmp") || image.uri.ends_with(".gif"), "Cannot handle the image ({}) type {}.", image.uri, image.mimeType);
		...
		{
			MGN_PROFILE_SCOPE("Model::Model -> Create Scenes");
			mgnModel->m_Scenes.reserve(model.scenes.size());
			for (auto &scene: model.scenes) {
				mgnModel->m_Scenes.push_back(scene.nodes);
			}
		}

		mgnModel->m_Materials.resize(model.materials.size());
		{
			MGN_PROFILE_SCOPE("Model::Model -> Create Meshes");
			mgnModel->m_Meshes.reserve(model.meshes.size());
			for (auto &mesh: model.meshes) {
				MGN_PROFILE_SCOPE("Model::Model -> Create MeshGroup");
				MeshGroup meshGroup;

				meshGroup.reserve(mesh.primitives.size());

				for (auto &primitive: mesh.primitives) {
					MGN_PROFILE_SCOPE("Model::Model -> Create Mesh");
					...
					{
						MGN_PROFILE_SCOPE("Model::Model -> Create Vertex RendererBuffer");
						...
						MGN_CORE_CHECK(primitive.mode == GLTF::MeshRenderMode::TRIANGLES, "The Render Mode {0} cannot be used for the moment.", primitive.mode);
						...
						MGN_CORE_CHECK(primitive.attributes.contains(positionAttribute), "A primitive must possess Positions.");
						MGN_CORE_CHECK(primitive.attributes.contains(normalAttribute), "A primitive must possess Normals.");
						MGN_CORE_CHECK(primitive.attributes.contains(texcoordAttribute), "A primitive must possess Texcoords.");
						...
						//Safeguards
						MGN_CORE_ASSERT(sizeof(glm::vec3) == sizeof(float) * 3, "glm::vec3 is not equal to 3 floats...");
						MGN_CORE_ASSERT(sizeof(glm::vec2) == sizeof(float) * 2, "glm::vec2 is not equal to 2 floats...");
						...
						MGN_CORE_ASSERT(positionsCount == normalsCount && normalsCount == texcoordsCount, "The buffer are not the same size");
					...
					{
						MGN_PROFILE_SCOPE("Model::Model -> Create Index RendererBuffer");
						if(primitive.indices > 0) {
							...
							switch ((GLTF::ComponentType) accessor.componentType) {
								case GLTF::ComponentType::UnsignedByte: {
									MGN_PROFILE_SCOPE("Model::Model -> Create Index RendererBuffer -> UnsignedByte");
								...
								case GLTF::ComponentType::UnsignedShort: {
									MGN_PROFILE_SCOPE("Model::Model -> Create Index RendererBuffer -> UnsignedShort");
								...
								case GLTF::ComponentType::UnsignedInt: {
									MGN_PROFILE_SCOPE("Model::Model -> Create Index RendererBuffer -> UnsignedInt");
								...
								default: MGN_CORE_ASSERT(false, "The component type {0} is not supported for an index buffer.", GLTF::Helper::ComponentTypeToString((GLTF::ComponentType) accessor.componentType));
							}
						}
						else {
							MGN_CORE_CHECK(vertexes.size() % 3 == 0, "There is no index buffer and the number of vertex is not divisible by 3 ({})", vertexes.size());
					...
					// Materials
					{
						MGN_PROFILE_SCOPE("Model::Model -> Create Material");
						if (primitive.material > -1) {
							tinygltf::Material mat = model.materials[primitive.material];

							std::string matName = std::to_string(modelId) + "_" + mat.name;

							if (!mgnModel->m_Materials[primitive.material]) {

								MGN_CORE_INFO("There is {0} material, we want to set the index {1}.", mgnModel->m_Materials.size(), primitive.material);
								...
								mgnModel->m_Materials[primitive.material] = material;
							}
							else {
								material = mgnModel->m_Materials[primitive.material];
							}
						}
						else {
							if(mgnModel->m_Materials.size() <= model.materials.size()) {
								MaterialParameters materialParams;
								material = assetManager->CreateMemoryAsset<Material>("Default Material", shader, materialParams);
								mgnModel->m_Materials.push_back(material);
							} else {
								material = mgnModel->m_Materials[model.materials.size()];
							}
						}
					}
				...
				mgnModel->m_Meshes.push_back(meshGroup);
			}
		}
