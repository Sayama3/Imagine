...
namespace Imagine::Core
{
	...
	Ref<Texture2D> TextureImporter::LoadTexture2D(const std::filesystem::path& path)
	{
		...
		if(!FileSystem::Exist(path)) {
			MGN_CORE_ERROR("The file {0} do not exist.", strPath);
			return nullptr;
		}

		{
			MGN_PROFILE_SCOPE("ImportTexture2D - stbi_load");
			buffer.Data = stbi_load(strPath.c_str(), &width, &height, &channels, 0);
		}

		if(!buffer.Data) {
			MGN_CORE_ERROR("Load of image '{0}' failed.\n{1}.", strPath, stbi_failure_reason());
			return nullptr;
		}
	...
	{
		MGN_CORE_INFO("Nothing to export on a texture 2D yet.");
	}
	...
	{
		MGN_CORE_INFO("Nothing to export on a cubemap yet.");
	}
	...
	Ref<Cubemap> TextureImporter::LoadCubemap(const std::filesystem::path &path)
	{
		...
		if(!raw) {
			MGN_CORE_ERROR("Load of cubemap '{0}' failed.\n{1}.", strPath, stbi_failure_reason());
			return nullptr;
		}
		...
		if(width > height) {
			if (width / 4 == height / 3) {
				...
				MGN_CORE_ERROR("Invalid cubemap dimensions: {0}x{1}.", width, height);
				return nullptr;
			}
		} else /*width < height*/{
			if (width / 3 == height / 4) {
